...
const DUMMY_PRODUCTS_URL = 'https://dummyjson.com/products?limit=30'
...
const mapDummyProduct = (product) => {
  ...
  return {
    ...
    company: product.brand || product.category || 'Sneakers',
    title: product.title,
    desc: product.description,
    img: product.images?.length ? product.images : [product.thumbnail],
    alt: product.title,
    categories: [
      {
        color: ['multi-color'],
...
export const getProductsByCollection = createAsyncThunk(
  ...
  async (collection) => {
    ...
    return products.filter((product) =>
      product.company.toLowerCase() === collection.toLowerCase()
    )
  })
